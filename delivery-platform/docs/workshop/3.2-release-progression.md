...
TBD<screenshot of the pipeline>
...
Click on the promote to prod link on the box lebelled prod. Once the pipeline finishes, review the page live by creating your tunnel
```bash
kubectx prod \
 && kubectl port-forward --namespace hello-web $(kubectl get pod --namespace hello-web --selector="app=hello-web,role=backend" --output jsonpath='{.items[0].metadata.name}') 8080:8080
 ```
